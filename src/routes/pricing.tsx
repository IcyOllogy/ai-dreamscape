...
export const Route = createFileRoute("/pricing")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Three ways into Noctis. Whisper, Intimate, Devotion. Cancel anytime.",
      },
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="pt-36 pb-16 max-w-5xl mx-auto px-6 text-center">
        ...
        <h1 className="font-display text-6xl md:text-7xl text-ivory leading-tight">
          Three ways
          <br />
          <em className="italic">in.</em>
        </h1>
        <p className="mt-6 text-muted-foreground max-w-xl mx-auto">
          No contracts. Cancel from your account at any moment, no awkwardness, no hold music.
        </p>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-10 pb-32">
        <div className="grid md:grid-cols-3 gap-6">
          {tiers.map((t) => (
            <div
              key={t.name}
              className={`relative p-10 ${t.featured ? "gold-border bg-card glow-gold scale-[1.02]" : "border border-border bg-card/30"}`}
            >
              {t.featured && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 text-[9px] uppercase tracking-[0.3em] bg-primary text-primary-foreground">
                  Most chosen
                </div>
              )}
              ...
              <Link
                to="/signup"
                className={`mt-10 block text-center py-3 text-xs uppercase tracking-[0.3em] transition-all ${t.featured ? "bg-primary text-primary-foreground hover:bg-primary/90" : "gold-border text-ivory hover:bg-ivory hover:text-background"}`}
              >
                {t.cta}
              </Link>
            </div>
        ...
        <div className="mt-20 text-center text-xs text-muted-foreground max-w-2xl mx-auto">
          All plans include private, encrypted conversations. Noctis is for adults only — every
          member must be 18+ in their jurisdiction.
        </div>
      </section>
      <SiteFooter />
    </div>
